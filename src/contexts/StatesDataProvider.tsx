...
function StatesDataProvider({ children }: PropsWithChildren<{}>) {
  ...
  const fetchStateData = async (state: StateAbbrs) => {
    ...
    try {
      const response = await fetch(`https://api.covidtracking.com/v2/states/${state}/daily/simple.json`);
      ...
      if (response.status === 200) {
        ...
        const covidData: CovidData[] = results.data.map(({ cases, date, outcomes, tests }: any) => ({
          state,
          date: date,
          currentIcuCount: outcomes.hospitalized.in_icu.currently,
          currentVentilatorCount: outcomes.hospitalized.on_ventilator.currently,
        ...
        setStates(prevStates => prevStates.filter(({ abbr }: State) => abbr !== state));
        setSortOrder('');
      } else {
  ...
  const removeStateData = (state: StateAbbrs) => {
    setIsRemoving(true);
    setStatesData(prevStatesData => prevStatesData.filter(data => data.state !== state));
    setStates(prevStates => {
      const stateList = [
        ...prevStates,
        {
          abbr: StateAbbrs[state],
